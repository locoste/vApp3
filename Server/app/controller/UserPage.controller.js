app.controller('UserPage', function($scope, $http) {
  ...
  $http.get('http://localhost:8002/getUserCompany').then(function(response){
    $scope.User = response.data[0].company;
    $scope.company = response.data[0].company;
    $http.get('http://localhost:8002/getUserInformation').then(function(responseUser){
      $scope.login = responseUser.data[0].login;
      $http.get('http://localhost:8002/getCompanyInformation').then(function(responseCompany){
  ...
  $scope.ChangeUser = function()
  {
    if($scope.password==$scope.repassword){
      ...
      $http.post('http://localhost:8002/updateUser', body).then(alert('user upadted with success'))
    }
    else
    {
